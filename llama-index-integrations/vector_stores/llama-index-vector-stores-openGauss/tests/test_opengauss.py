...
@pytest.fixture()
def pg_hnsw_multiple(db_hnsw: None) -> Generator[List[OpenGaussStore], None, None]:
    """
    This creates multiple instances of OpenGaussStore.
    """
    pgs = []
    for _ in range(2):
        pg = OpenGaussStore.from_params(
...
async def test_instance_creation(db: None) -> None:
    ...
    assert isinstance(pg, OpenGaussStore)
...
async def test_add_to_db_and_query(
    pg_fixture: OpenGaussStore, node_embeddings: List[TextNode], use_async: bool
) -> None:
    if use_async:
        await pg_fixture.async_add(node_embeddings)
    else:
        pg_fixture.add(node_embeddings)
    assert isinstance(pg_fixture, OpenGaussStore)
...
async def test_query_hnsw(
    pg_hnsw: OpenGaussStore, node_embeddings: List[TextNode], use_async: bool
):
    if use_async:
    ...
    assert isinstance(pg_hnsw, OpenGaussStore)
...
async def test_hybrid_query(
    ...
    assert isinstance(pg_hnsw_hybrid, OpenGaussStore)
...
def test_hybrid_query_fails_if_no_query_str_provided(
    pg_hybrid: OpenGaussStore, hybrid_node_embeddings: List[TextNode]
) -> None:
    q = VectorStoreQuery(
...
async def test_add_to_db_and_query_index_nodes(
    pg_fixture: OpenGaussStore, index_node_embeddings: List[BaseNode], use_async: bool
) -> None:
    if use_async:
...
async def test_delete_nodes(
    pg_fixture: OpenGaussStore, node_embeddings: List[BaseNode], use_async: bool
) -> None:
    if use_async:
        await pg_fixture.async_add(node_embeddings)
    else:
...
async def test_hnsw_index_creation(
    pg_hnsw_multiple: List[OpenGaussStore],
    node_embeddings: List[TextNode],
    use_async: bool,
) -> None:
    """
    This test will make sure that creating multiple OpenGaussStores handles db initialization properly.
    """
    ...
    # these are the actual table and index names that OpenGaussStore automatically created
...
async def test_custom_engines(db: None, node_embeddings: List[TextNode]) -> None:
    """Test that OpenGaussStore works correctly with custom engines."""
    ...
    # Create OpenGaussStore with custom engines
    pg = OpenGaussStore(
        embed_dim=TEST_EMBED_DIM,
        engine=engine,
...
def test_custom_sync_engine_only(db: None, node_embeddings: List[TextNode]) -> None:
    """Test that OpenGaussStore works correctly with only a custom sync engine."""
    from sqlalchemy import create_engine
    ...
    # Create OpenGaussStore with custom sync engine only
...
async def test_custom_async_engine_only(
    ...
    """Test that OpenGaussStore works correctly with only a custom async engine."""
    from sqlalchemy.ext.asyncio import create_async_engine
    ...
    # Create OpenGaussStore with custom async engine only
    with pytest.raises(ValueError):
        _ = OpenGaussStore(
            embed_dim=TEST_EMBED_DIM,
            async_engine=async_engine,
